...
describe('filter', () => {
   ...
   it("Should work, when callback return non-standard values", () => {
      ...
      assert.deepEqual(filter(array, item => 0), [])
      assert.deepEqual(filter(array, item => 1), array)
      assert.deepEqual(filter(array, item => ''), [])
      assert.deepEqual(filter(array, item => '0'), array)
   })
